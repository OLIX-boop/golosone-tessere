...
distingue lo schermo dalla carta, quindi li' non cambia niente. Da li' si
aggiunge anche la tessera al Wallet, con `/c/CODICE/apple` e
`/c/CODICE/wallet`.
...
### Due strade, rimesse insieme

Fra il 19 e il 26 settembre 2026 il progetto e' andato avanti su due copie
separate, e in produzione ha girato ora l'una ora l'altra. Il 26 sono state
unite, ma i pass gia' nei telefoni se le portano dietro: indirizzo di
aggiornamento e token sono scritti dentro il pass, e non si cambiano.

- Il servizio di aggiornamento risponde **a due indirizzi**: `/wallet-apple`
  (quello di adesso) e `/api/apple/v1` (i pass emessi fra il 21 e il 26).
- Accetta **due token**: quello di adesso, derivato da `apple_auth_key`, e
  quello dei pass del 21-26, `sha256(apple_auth_secret:codice)`. Le due righe
  in `settings` vanno lasciate dove sono.
- Il certificato arriva in **due forme**, e in produzione ci sono tutte e due:
  la coppia `APPLE_PASS_CERT` + `APPLE_PASS_KEY` (+ `APPLE_WWDR_CERT`, con
  Pass Type ID e Team ID nelle impostazioni) e il pacchetto
  `APPLE_WALLET_CERT`. **Vince la coppia**, che e' quella con cui la tessera
  e' stata provata dall'app; dal pacchetto si prende la chiave del push.

I test in `test/apple-wallet.test.ts` («le due strade, rimesse insieme»)
tengono ferme queste tre cose.
...
che stanno in `src/pass-comune.ts`. Lo stile `golosone` li prende gia' da
li'; per gli altri lo script li stampa a fine esecuzione, pronti da incollare.
...
Per Google lo script fa anche `logo-largo.png` (1280x400), che prende il posto
del logo tondo in cima alla tessera: il marchio intero, in alto a sinistra
come su Apple.

Il **sito** invece resta sul prugna misurato sui pixel pieni del logo,
`#56343c`, in `public/tema.css`: cassa, pannello titolare e pagina cliente.
...
La classe di Google si riallinea da sola al primo «Aggiungi a Google Wallet»
dopo un cambiamento: nome, colore, loghi ed etichette. Gli indirizzi delle
immagini portano un'impronta del loro contenuto, perche' Google tiene in cache
per indirizzo: cambiato il file, cambia l'indirizzo, e la classe se ne
accorge.
